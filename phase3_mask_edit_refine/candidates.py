...
def compile_directional_tapered_projection_field(
    ...
    *,
    ...
    if centerline_first:
        # A partial cord budget must first establish longitudinal extent and
        # a one-cell-wide proximal seam pocket, then widen the remaining
        # envelope.  This keeps the shaft elongated while admitting one
        # complete source-calibrated nucleus at its attachment.
        ...
        pocket_distance = np.sqrt(
            np.square(np.maximum(longitudinal, 0.0) / pocket_longitudinal_radius)
        ...
        chain_distance = np.sqrt(
            np.square(
                (longitudinal - chain_center) / chain_longitudinal_radius
            )
