...
def _check_execution_contract_fidelity(context: GateContext) -> GateCheck:
    ...
    for index, item in enumerate(planned):
        ...
        item_passed = (
            ...
            and (
                residual_fragmentation
                or directional_centerline_first
...
def _check_depth_span_ratio(context: GateContext) -> GateCheck:
    ...
    # A source-scaled narrow cord is intentionally deep relative to its small
    # attachment span.  Its dedicated raster audit owns elongation, taper,
    # branching and side-merge safety; this legacy broad-front ratio would
    # otherwise reject the desired geometry.  Keep the absolute depth band.
    passed = max_depth <= band_max + 1e-6 and (
