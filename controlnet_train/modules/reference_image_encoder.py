...
class ReferenceImageEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.reference_ip_embedding_backend = "uni"
    ...
    def _validate_feature_layer(self) -> None:
        ...
        if self.feature_layer <= 0:
            raise ValueError(f"UNI feature_layer must be 1-based and positive, got {self.feature_layer}.")
        if not hasattr(self.uni, "get_intermediate_layers"):
            raise RuntimeError("UNI backbone does not expose get_intermediate_layers(); cannot use feature_layer.")
        depth = len(getattr(self.uni, "blocks", []))
        if depth > 0 and self.feature_layer > depth:
            raise ValueError(f"UNI feature_layer exceeds backbone depth {depth}: {self.feature_layer}.")
    ...
    def extract_uni_features(
        ...
        *,
        ...
        def _extract() -> torch.Tensor:
            ...
            layer = self.feature_layer if feature_layer is None else feature_layer
            if layer is None:
                features = self.uni.forward_features(x)
            else:
                features = self._extract_uni_intermediate_features(x, int(layer))
            return self._normalize_uni_token_output(features, x)
    ...
    def _extract_uni_intermediate_features(self, x: torch.Tensor, layer_number: int) -> torch.Tensor:
        if layer_number <= 0:
            raise ValueError(f"UNI feature_layer must be 1-based and positive, got {layer_number}.")
        if not hasattr(self.uni, "get_intermediate_layers"):
            raise RuntimeError("UNI backbone does not expose get_intermediate_layers(); cannot use feature_layer.")
        depth = len(getattr(self.uni, "blocks", []))
        if depth > 0 and layer_number > depth:
            raise ValueError(f"UNI feature_layer exceeds backbone depth {depth}: {layer_number}.")
        outputs = self.uni.get_intermediate_layers(
            x,
            n=[int(layer_number) - 1],
            reshape=False,
        )
        if not outputs:
    ...
    def _normalize_uni_token_output(self, features: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        if features.ndim == 4:
            features = features.flatten(2).transpose(1, 2)
        if features.ndim != 3:
            raise ValueError(f"UNI features must be BxTxC or BxCxHxW, got {tuple(features.shape)}")
        patch_size = int(UNI2H_KWARGS["patch_size"])
    ...
    def forward(self, images: torch.Tensor, *, reference_images: torch.Tensor | None = None) -> torch.Tensor:
...
class ReferenceVaeLatentEncoder(nn.Module):
    """Project frozen VAE reference latents into regional IP-Adapter tokens.

    The input is expected to be a deterministic FLUX VAE latent grid shaped
    ``(B, C, H, W)``. A 2x2 local pack turns it into a square sequence, e.g.
    512px RGB -> 64x64 latent -> 32x32 reference tokens. The regional IP mask
    builder can then label these tokens from tissue/nuclei masks exactly like
    UNI spatial tokens, but at a higher spatial resolution and closer to RGB.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.use_perceiver_self_attn = bool(use_perceiver_self_attn)
        self.skip_perceiver = bool(skip_perceiver)
        self.reference_ip_embedding_backend = "vae_latent"
        self.perceiver_cross_gate_init = (
            None if perceiver_cross_gate_init is None else float(perceiver_cross_gate_init)
        )
        if self.token_grid_size <= 0:
            raise ValueError(f"token_grid_size must be positive, got {self.token_grid_size}.")
        if self.pack_factor <= 0:
            raise ValueError(f"pack_factor must be positive, got {self.pack_factor}.")

        input_dim = self.latent_channels * self.pack_factor * self.pack_factor
    ...
    def forward(self, latents: torch.Tensor, *, reference_images: torch.Tensor | None = None) -> torch.Tensor:
        projected = self.encode_projected_patch_tokens(latents)
        ref_gate = self.reference_presence_gate(
            latents if reference_images is None else reference_images,
            device=projected.device,
            dtype=projected.dtype,
        )
        if self.skip_perceiver:
            return projected * ref_gate
        return self._resample(projected) * ref_gate

    def encode_projected_patch_tokens(self, latents: torch.Tensor) -> torch.Tensor:
        packed = self._pack_latents(latents)
        proj_dtype = next(self.proj_mlp.parameters()).dtype
        return self.proj_mlp(packed.to(dtype=proj_dtype))
    ...
    def reference_presence_gate(
        ...
        *,
        ...
        if tensor.ndim < 2:
            raise ValueError(f"reference gate input must have at least 2 dims, got {tuple(tensor.shape)}")
        detached = tensor.detach()
        reduce_dims = tuple(range(1, detached.ndim))
        gate = (detached.abs().amax(dim=reduce_dims) > float(eps)).to(
            device=detached.device if device is None else device,
            dtype=detached.dtype if dtype is None else dtype,
        )
        return gate.view(-1, 1, 1)

    def load_perceiver_layers_state_dict(self, state_dict: dict[str, torch.Tensor]) -> None:
        incompatible = self.perceiver_layers.load_state_dict(state_dict, strict=False)
        allowed_missing = set()
        if self.perceiver_cross_gate_init is not None:
            allowed_missing = {
        ...
        if missing or incompatible.unexpected_keys:
            raise RuntimeError(
                "Reference VAE Perceiver checkpoint mismatch: "
                f"missing={missing}, unexpected={incompatible.unexpected_keys}"
            )

    def _pack_latents(self, latents: torch.Tensor) -> torch.Tensor:
        if latents.ndim != 4:
            raise ValueError(f"VAE latents must have shape (B,C,H,W), got {tuple(latents.shape)}")
        if latents.shape[1] != self.latent_channels:
            raise ValueError(
                f"VAE latent channel mismatch: got {latents.shape[1]}, expected {self.latent_channels}."
            )
        batch, channels, height, width = latents.shape
        pack = self.pack_factor
        expected_h = self.token_grid_size * pack
        expected_w = self.token_grid_size * pack
        if height != expected_h or width != expected_w:
            raise ValueError(
                "VAE latent grid does not match configured reference token grid: "
                f"got {height}x{width}, expected {expected_h}x{expected_w} "
                f"for token_grid_size={self.token_grid_size}, pack_factor={pack}."
            )
        x = latents.float()
        x = x.view(batch, channels, height // pack, pack, width // pack, pack)
        x = x.permute(0, 2, 4, 1, 3, 5)
        return x.reshape(batch, (height // pack) * (width // pack), channels * pack * pack)

    def _resample(self, projected: torch.Tensor) -> torch.Tensor:
        batch = projected.shape[0]
        latents = self.latent_queries.expand(batch, -1, -1)
        for layer in self.perceiver_layers:
            latents = layer(latents, projected)
        return self.perceiver_norm(latents)
        ...
        *,
        ...
        for sample_index in range(projected.shape[0]):
            ...
            if not region_tokens:
                pooled = projected.new_zeros((1, 1, projected.shape[-1]))
                region_tokens.append(pooled[0])
                region_labels.append(torch.full((pooled.shape[1],), -1, dtype=torch.long, device=projected.device))
            batch_tokens.append(torch.cat(region_tokens, dim=0))
            batch_labels.append(torch.cat(region_labels, dim=0))
        return _pad_region_token_batches(batch_tokens, batch_labels, projected)
        ...
        *,
        ...
        for sample_index in range(projected.shape[0]):
            ...
            for label in unique_labels:
                ...
                std_token = region_values.float().std(dim=0, unbiased=False).to(dtype=projected.dtype)
                region_tokens.append(torch.stack([mean_token, std_token], dim=0))
                region_labels.append(torch.full((2,), label_value, dtype=torch.long, device=projected.device))
            if not region_tokens:
                region_tokens.append(projected.new_zeros((1, projected.shape[-1])))
                region_labels.append(torch.full((1,), -1, dtype=torch.long, device=projected.device))
            batch_tokens.append(torch.cat(region_tokens, dim=0))
            batch_labels.append(torch.cat(region_labels, dim=0))
        return _pad_region_token_batches(batch_tokens, batch_labels, projected)
...
def _pad_region_token_batches(
    batch_tokens: list[torch.Tensor],
    batch_labels: list[torch.Tensor],
    reference: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    max_tokens = max(tokens.shape[0] for tokens in batch_tokens)
    padded_tokens = reference.new_zeros((reference.shape[0], max_tokens, reference.shape[-1]))
    padded_labels = torch.full(
        (reference.shape[0], max_tokens),
        -1,
        dtype=torch.long,
