"""Evaluate a Phase 5.3 Cross V3 FLUX ControlNet."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate Phase 5.3 Cross V3 FLUX ControlNet.")
    ...
    parser.add_argument(
        ...
        default="fixed",
        help="Deprecated for Cross V3; inference always uses the fixed prompt.",
    )
    parser.add_argument("--prompt", default=None, help="Deprecated for Cross V3; ignored.")
    parser.add_argument(
        "--color-match",
        choices=("none", "lab"),
        default="lab",
...
def main(argv=None) -> int:
    ...
    for index, record in enumerate(records):
        ...
        for variant in _reference_variants(args.run_zero_ref_ablation):
            ...
            if fixed_t_eval_timesteps:
                with torch.no_grad():
                    fixed_t_losses = compute_cross_v3_fixed_timestep_losses(
                        bundle,
                        reference_image=reference_image_tensor,
                        reference_tissue_mask=reference_tissue_mask,
                        reference_nuclei_mask=reference_nuclei_mask,
            ...
            if args.run_zero_ref_ablation:
                ...
                metric_row["zero_ref_ablation"] = variant == CROSS_V3_REFERENCE_ZERO_REF
        ...
        if args.run_zero_ref_ablation:
            ...
            zero_metrics = next(
                result["metrics"] for result in variant_results if result["variant"] == CROSS_V3_REFERENCE_ZERO_REF
            )
    ...
    summary["prompt_policy"] = {"kind": "fixed", "prompt": CROSS_V3_PROMPT}
    if fixed_t_rows:
...
def _reference_variants(run_zero_ref_ablation: bool) -> list[str]:
    ...
    variants = [CROSS_V3_REFERENCE_WITH_REF]
    if run_zero_ref_ablation:
        variants.append(CROSS_V3_REFERENCE_ZERO_REF)
    return variants
