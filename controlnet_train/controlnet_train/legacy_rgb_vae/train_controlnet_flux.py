...
================================
9通道 ControlNet: [reference_image + target_mask + reference_mask] → 生成 target_image
...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            while len(models) > 0:
                model = models.pop()
                cn_dir = os.path.join(input_dir, "flux_controlnet")
                shard_files = sorted(glob.glob(os.path.join(cn_dir, "diffusion_pytorch_model*.safetensors")))
                if not shard_files:
                    shard_files = sorted(glob.glob(os.path.join(cn_dir, "diffusion_pytorch_model*.bin")))
                state_dict = {}
