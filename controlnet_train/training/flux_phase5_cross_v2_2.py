"""Phase 5.3 Cross V2.2 training for Flux ControlNet.

Cross V2.2 removes the Cross V1 IP-Adapter path. Reference appearance enters
...
def collate_cross_v2_2_batch(examples: list[dict]) -> dict:
    flattened: list[dict] = []
...
def _build_cross_v2_2_control_batch(
    *,
    batch: dict,
    modules: dict[str, nn.Module],
    vae: AutoencoderKL,
    weight_dtype: torch.dtype,
    reference_bank_block_size: int,
    reference_bank_label_mode: str,
    zero_reference_mask_features: bool,
    ...
    reference_bank_latent = build_cross_v2_2_block_bank_reference_latent(
        z_ref=reference_image_latent,
        reference_tissue_mask=batch["reference_tissue_mask"],
        reference_nuclei_mask=batch["reference_nuclei_mask"],
        target_tissue_mask=batch["target_tissue_mask"],
        target_nuclei_mask=batch["target_nuclei_mask"],
        block_size=reference_bank_block_size,
        label_mode=reference_bank_label_mode,
    ).to(dtype=weight_dtype)
    if zero_reference_mask_features:
        ref_tissue_feat = torch.zeros_like(ref_tissue_feat)
        ref_nuclei_feat = torch.zeros_like(ref_nuclei_feat)

    control_tensor = build_cross_v2_2_condition(
        z_ref=reference_bank_latent,
...
def _save_condition_modules(
    ...
    *,
    control_spec: CrossV22ControlSpec,
    reference_bank_block_size: int,
    reference_bank_label_mode: str,
    zero_reference_mask_features: bool,
) -> None:
    state = {
        ...
        "cross_v2_2_control_spec": {
            ...
            "reference_bank_block_size": int(reference_bank_block_size),
            "reference_bank_label_mode": str(reference_bank_label_mode),
            "zero_reference_mask_features": bool(zero_reference_mask_features),
        },
    }
...
def _save_cross_v2_2_artifacts(
    output_dir: str,
    ...
    *,
    ...
    _save_condition_modules(
        ...
        reference_bank_block_size=max(1, int(getattr(args, "reference_bank_block_size", 4) or 4)),
        reference_bank_label_mode=str(getattr(args, "reference_bank_label_mode", "tissue_nuclei")),
        zero_reference_mask_features=not bool(getattr(args, "keep_reference_mask_features", False)),
    )


def _load_cross_v2_2_controlnet_checkpoint(
    checkpoint_path: str | Path,
    control_spec: CrossV22ControlSpec,
) -> FluxControlNetModel:
    checkpoint = _resolve_controlnet_artifact_path(Path(checkpoint_path))
    logger.info("Loading Cross V2.2 ControlNet warm start from %s", checkpoint)
...
def _load_source_layout(checkpoint: Path) -> dict:
    ...
    if "cross_v2_2_control_spec" in state:
        return {"kind": "cross_v2_2", **dict(state["cross_v2_2_control_spec"])}
...
def _remap_x_embedder_state_dict(
    ...
    *,
    source_layout: dict,
    target_spec: CrossV22ControlSpec,
...
def run_cross_v2_2_training(args: argparse.Namespace) -> None:
    cross_version = str(getattr(args, "cross_version", "v2.2")).lower().replace("_", ".").replace("-", ".")
    if cross_version not in {"v2.2", "v22"}:
        raise NotImplementedError("This module implements only cross V2.2.")
    ...
    reference_bank_block_size = max(1, int(getattr(args, "reference_bank_block_size", 4) or 4))
    reference_bank_label_mode = str(
        getattr(args, "reference_bank_label_mode", "tissue_nuclei") or "tissue_nuclei"
    ).strip().lower().replace("-", "_")
    if reference_bank_label_mode not in {"tissue", "nuclei", "tissue_nuclei"}:
        raise ValueError(
            "--reference-bank-label-mode must be one of: tissue, nuclei, tissue_nuclei."
        )
    zero_reference_mask_features = not bool(getattr(args, "keep_reference_mask_features", False))
    ...
    logger.info(
        "Using Cross V2.2 condition order [z_ref_bank, ref_tissue_feat, ref_nuclei_feat, "
        "tar_tissue_feat, tar_nuclei_feat]: raw_channels=%s packed_channels=%s",
        control_spec.raw_channels,
        control_spec.packed_channels,
    )
    logger.info(
        "Using Cross V2.2 reference block bank: block_size=%s label_mode=%s "
        "zero_reference_mask_features=%s. Degrade/noising path remains enabled via "
        "clean_image_for_noising and uses_degraded_noising.",
        reference_bank_block_size,
        reference_bank_label_mode,
        zero_reference_mask_features,
    )
    ...
    if args.controlnet_model_name_or_path:
        flux_controlnet = _load_cross_v2_2_controlnet_checkpoint(
    ...
    logger.info("Patched controlnet_x_embedder to packed width %s for cross-v2.2", control_spec.packed_channels)
    ...
    dataloader_kwargs = {
        "shuffle": True,
        "collate_fn": collate_cross_v2_2_batch,
    ...
    logger.info("***** Running Phase 5.3 cross-v2.2 training *****")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_controlnet):
                ...
                pixel_latents, noising_latents, control_tensor = _build_cross_v2_2_control_batch(
                    batch=training_batch,
                    modules=modules,
                    vae=vae,
                    weight_dtype=weight_dtype,
                    reference_bank_block_size=reference_bank_block_size,
                    reference_bank_label_mode=reference_bank_label_mode,
                    zero_reference_mask_features=zero_reference_mask_features,
                )
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process and global_step % args.checkpointing_steps == 0:
                    ...
                    logger.info("Saved eval-ready Phase 5.3 cross-v2.2 artifacts to %s", save_path)
    ...
    if accelerator.is_main_process:
        ...
        logger.info("Saved Phase 5.3 cross-v2.2 artifacts to %s", args.output_dir)
