...
class SameWSIPairClassifier(nn.Module):
    ...
    def __init__(self, encoder: SameWSIAppearanceEncoder | None = None) -> None:
        ...
        dim = int(self.encoder.config.embedding_dim)
    ...
    def forward(self, image_a: torch.Tensor, image_b: torch.Tensor) -> torch.Tensor:
        ...
        pairs = torch.cat([emb_a, emb_b, torch.abs(emb_a - emb_b)], dim=-1)
        return self.head(pairs).flatten()
...
def same_wsi_perceptual_loss(
    *,
    ...
    if input_size > 0 and tuple(prediction.shape[-2:]) != (input_size, input_size):
    ...
    for layer_index in feature_layers:
        if layer_index < 0 or layer_index >= len(pred_features):
            raise ValueError(
                f"same-WSI feature layer {layer_index} is out of range for "
                f"{len(pred_features)} blocks."
            )
...
def load_same_wsi_encoder(
    ...
    if not isinstance(checkpoint, dict):
        raise ValueError(f"same-WSI checkpoint must be a dict, got {type(checkpoint).__name__}.")

    config = _config_from_checkpoint(checkpoint.get("config", {}))
    encoder = SameWSIAppearanceEncoder(config)
    state = _select_encoder_state_dict(checkpoint)
...
def _masked_feature_distance(
    *,
    ...
    reference_small = F.interpolate(
        reference_mask.unsqueeze(1).float(),
        size=size,
        mode="nearest",
    ).squeeze(1).long()
...
def _config_from_checkpoint(config: dict) -> SameWSIAppearanceConfig:
    values = dict(config or {})
    for key in ("backbone_channels", "feature_layers", "normalize_mean", "normalize_std"):
        if key in values:
            values[key] = tuple(values[key])
    return SameWSIAppearanceConfig(**values)


def _select_encoder_state_dict(checkpoint: dict) -> dict[str, torch.Tensor]:
    state = (
        checkpoint.get("encoder")
    ...
    if not isinstance(state, dict):
        raise ValueError(f"same-WSI encoder state must be a dict, got {type(state).__name__}.")
