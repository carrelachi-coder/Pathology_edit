...
# Phase 5.3 Cross V3.
# Prompt is fixed to "histopathology image". ControlNet sees only target
# structure [tar_tissue_feat, tar_nuclei_feat]; reference z_ref+mask are
# projected to FLUX joint cross-attention tokens.
...
CROSS_V3_OUTPUT_DIR="${CROSS_V3_OUTPUT_DIR:-/data/wqx/flowedit/controlnet_cross_v3_nohed_selfrec}"

# Optional warm start from Cross V1, V2.1, or V3. Only the source target-mask
# projection slice is remapped into the v3 target-only ControlNet input.
...
REFERENCE_ROUTE_EMBEDDING_INIT_STD="${REFERENCE_ROUTE_EMBEDDING_INIT_STD:-0.02}"
...
TARGET_TISSUE_EMBEDDING_DIM="${TARGET_TISSUE_EMBEDDING_DIM:-}"
...
TRAIN_TARGET_TISSUE_ARGS=()
TRAIN_TARGET_TISSUE_ARGS+=(--target-tissue-encoding "${TARGET_TISSUE_ENCODING}")
TRAIN_TARGET_TISSUE_ARGS+=(--target-one-hot-scale "${TARGET_ONE_HOT_SCALE}")
if [[ -n "${TARGET_TISSUE_EMBEDDING_DIM}" ]]; then
  TRAIN_TARGET_TISSUE_ARGS+=(--target-tissue-embedding-dim "${TARGET_TISSUE_EMBEDDING_DIM}")
fi
...
accelerate launch --multi_gpu --num_processes="${NUM_PROCESSES}" --gpu_ids="${GPU_IDS}" \
  ...
  "${TRAIN_HED_ARGS[@]}" \
  "${TRAIN_CHECKPOINT_ARGS[@]}" \
  --output-dir "${CROSS_V3_OUTPUT_DIR}" \
  "${RESUME_ARGS[@]}" \
  --logging-dir logs \
  --seed 42 \
  --train-batch-size 1 \
  ...
  --reference-route-embedding-init-std "${REFERENCE_ROUTE_EMBEDDING_INIT_STD}" \
  ...
  --tracker-project-name flux_controlnet_phase5_cross_v3_nohed_selfrec
