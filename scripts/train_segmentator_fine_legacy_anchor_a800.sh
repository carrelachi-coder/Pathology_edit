...
OUTPUT_DIR="${OUTPUT_DIR:-/data1/zhao/wqx/segmentator_fine/legacy_anchor_fine_seed42}"
COARSE_CHECKPOINT="${COARSE_CHECKPOINT:-/data1/zhao/wqx/segmentator/best_mIoU.pt}"
...
if [[ "$RESUME_FROM_CHECKPOINT" == "latest" && -f "$OUTPUT_DIR/checkpoint_last.pt" ]]; then
  ...
  echo "Legacy coarse checkpoint not found: $COARSE_CHECKPOINT" >&2
  exit 2
fi
...
echo "Launching legacy-anchor Fine training on GPUs $GPU_IDS (world_size=$NUM_GPUS)"
if (( NUM_GPUS > 1 )); then
