...
OUTPUT_DIR="${OUTPUT_DIR:-/data1/zhao/wqx/segmentator_fine/legacy_anchor_joint_seed42}"
FINE_CHECKPOINT="${FINE_CHECKPOINT:-/data1/zhao/wqx/segmentator_fine/legacy_anchor_fine_seed42/best_composite.pt}"
...
if [[ "$RESUME_FROM_CHECKPOINT" == "latest" && -f "$OUTPUT_DIR/checkpoint_last.pt" ]]; then
  ...
  echo "Legacy-anchor Fine checkpoint not found: $FINE_CHECKPOINT" >&2
  exit 2
fi
...
TRAIN_ARGS=(
  ...
  --decoder mask2former
  --feature-pyramid-source legacy_final_depth
...
echo "Launching legacy-anchor Boundary + CellViT Teacher joint training on GPUs $GPU_IDS (world_size=$NUM_GPUS)"
if (( NUM_GPUS > 1 )); then
