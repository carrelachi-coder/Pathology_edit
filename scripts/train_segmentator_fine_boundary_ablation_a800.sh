...
OUTPUT_DIR="${OUTPUT_DIR:-/data1/zhao/wqx/segmentator_fine/fine_boundary_ablation_seed42}"
...
TRAIN_ARGS=(
  ...
  --rare-class-sampling
  --rare-class-ids 3 4 6
  --rare-class-sample-boost 2.0
  ...
  --refinement-loss-weight 1.0
  --refinement-boundary-weight 0.5
  --cellvit-mode none
  ...
  --augment-vflip
  --augment-rot90
  --augment-scale-crop 0.1
  --no-amp
  "${START_ARGS[@]}"
)

echo "Launching Fine Boundary ablation on GPUs $GPU_IDS (world_size=$NUM_GPUS)"
if (( NUM_GPUS > 1 )); then
