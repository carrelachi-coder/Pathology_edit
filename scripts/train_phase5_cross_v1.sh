...
CROSS_V1_OUTPUT_DIR="${CROSS_V1_OUTPUT_DIR:-/data/wqx/flowedit/controlnet_cross_v1_uni_l6_spatial_conch_l6_region_gram}"
...
REGIONAL_IP_EMBEDDING_BACKEND="${REGIONAL_IP_EMBEDDING_BACKEND:-${REFERENCE_IP_EMBEDDING_BACKEND:-uni}}"
REFERENCE_UNI_FEATURE_LAYER="${REFERENCE_UNI_FEATURE_LAYER:-6}"
REFERENCE_VAE_LATENT_CHANNELS="${REFERENCE_VAE_LATENT_CHANNELS:-16}"
REFERENCE_VAE_TOKEN_GRID_SIZE="${REFERENCE_VAE_TOKEN_GRID_SIZE:-32}"
REFERENCE_VAE_PACK_FACTOR="${REFERENCE_VAE_PACK_FACTOR:-2}"
REGIONAL_IP_TOKEN_MODE="${REGIONAL_IP_TOKEN_MODE:-spatial}"
...
A1_LITE="${A1_LITE:-1}"
if [[ -z "${LOAD_REF_ENCODER+x}" ]]; then
  if [[ "${REGIONAL_IP_EMBEDDING_BACKEND}" == "vae" || "${REGIONAL_IP_EMBEDDING_BACKEND}" == "vae_latent" || "${REGIONAL_IP_EMBEDDING_BACKEND}" == "latent" ]]; then
    LOAD_REF_ENCODER=0
  elif [[ -n "${REFERENCE_UNI_FEATURE_LAYER}" && "${REFERENCE_UNI_FEATURE_LAYER}" != "final" && "${REFERENCE_UNI_FEATURE_LAYER}" != "none" ]]; then
    LOAD_REF_ENCODER=0
  else
    LOAD_REF_ENCODER=1
  fi
...
REFERENCE_REGION_CONCH_LAYER="${REFERENCE_REGION_CONCH_LAYER:-6}"
...
REFERENCE_REGION_STD_WEIGHT="${REFERENCE_REGION_STD_WEIGHT:-1.0}"
REFERENCE_REGION_GRAM_WEIGHT="${REFERENCE_REGION_GRAM_WEIGHT:-0.05}"
...
REFERENCE_REGION_COSINE_WEIGHT="${REFERENCE_REGION_COSINE_WEIGHT:-0}"
...
REFERENCE_TEXTURE_LOSS_WEIGHT="${REFERENCE_TEXTURE_LOSS_WEIGHT:-0}"
REFERENCE_TEXTURE_LOSS_INTERVAL="${REFERENCE_TEXTURE_LOSS_INTERVAL:-4}"
REFERENCE_TEXTURE_LOSS_MIN_SIGMA="${REFERENCE_TEXTURE_LOSS_MIN_SIGMA:-0.0}"
REFERENCE_TEXTURE_LOSS_MAX_SIGMA="${REFERENCE_TEXTURE_LOSS_MAX_SIGMA:-0.6}"
REFERENCE_TEXTURE_TISSUE_WEIGHT="${REFERENCE_TEXTURE_TISSUE_WEIGHT:-1.0}"
REFERENCE_TEXTURE_NUCLEI_WEIGHT="${REFERENCE_TEXTURE_NUCLEI_WEIGHT:-0.25}"
REFERENCE_TEXTURE_COMPOSITE_WEIGHT="${REFERENCE_TEXTURE_COMPOSITE_WEIGHT:-0}"
REFERENCE_TEXTURE_MIN_PIXELS="${REFERENCE_TEXTURE_MIN_PIXELS:-8}"
...
REFERENCE_VGG_RGB="${REFERENCE_VGG_RGB:-0}"
...
echo "LOAD_REF_ENCODER=${LOAD_REF_ENCODER}"
...
echo "REGIONAL_IP_EMBEDDING_BACKEND=${REGIONAL_IP_EMBEDDING_BACKEND}"
echo "REFERENCE_UNI_FEATURE_LAYER=${REFERENCE_UNI_FEATURE_LAYER:-final}"
echo "REFERENCE_VAE_TOKEN_GRID_SIZE=${REFERENCE_VAE_TOKEN_GRID_SIZE}"
echo "REFERENCE_VAE_PACK_FACTOR=${REFERENCE_VAE_PACK_FACTOR}"
...
echo "REFERENCE_REGION_CONCH_LAYER=${REFERENCE_REGION_CONCH_LAYER:-final}"
...
echo "REFERENCE_REGION_GRAM_WEIGHT=${REFERENCE_REGION_GRAM_WEIGHT}"
echo "REFERENCE_TEXTURE_LOSS_WEIGHT=${REFERENCE_TEXTURE_LOSS_WEIGHT}"
echo "REFERENCE_TEXTURE_LOSS_INTERVAL=${REFERENCE_TEXTURE_LOSS_INTERVAL}"
echo "REFERENCE_TEXTURE_LOSS_SIGMA=[${REFERENCE_TEXTURE_LOSS_MIN_SIGMA},${REFERENCE_TEXTURE_LOSS_MAX_SIGMA}]"
echo "REFERENCE_VGG_LAYERS=${REFERENCE_VGG_LAYERS}"
echo "REFERENCE_VGG_LOSS_TYPE=${REFERENCE_VGG_LOSS_TYPE}"
echo "REFERENCE_VGG_RGB=${REFERENCE_VGG_RGB}"
...
TRAIN_REFERENCE_EMBEDDING_ARGS=(
  --reference-ip-embedding-backend "${REGIONAL_IP_EMBEDDING_BACKEND}"
  --reference-vae-latent-channels "${REFERENCE_VAE_LATENT_CHANNELS}"
  --reference-vae-token-grid-size "${REFERENCE_VAE_TOKEN_GRID_SIZE}"
  --reference-vae-pack-factor "${REFERENCE_VAE_PACK_FACTOR}"
)
if [[ -n "${REFERENCE_UNI_FEATURE_LAYER}" && "${REFERENCE_UNI_FEATURE_LAYER}" != "final" && "${REFERENCE_UNI_FEATURE_LAYER}" != "none" ]]; then
  TRAIN_REFERENCE_EMBEDDING_ARGS+=(--reference-uni-feature-layer "${REFERENCE_UNI_FEATURE_LAYER}")
fi
...
TRAIN_REGION_LOSS_ARGS=(
  ...
  --reference-region-gram-weight "${REFERENCE_REGION_GRAM_WEIGHT}"
...
if [[ -n "${REFERENCE_REGION_CONCH_LAYER}" && "${REFERENCE_REGION_CONCH_LAYER}" != "final" && "${REFERENCE_REGION_CONCH_LAYER}" != "none" ]]; then
  TRAIN_REGION_LOSS_ARGS+=(--reference-region-conch-layer "${REFERENCE_REGION_CONCH_LAYER}")
fi
...
TRAIN_TEXTURE_LOSS_ARGS=(
  --reference-texture-loss-weight "${REFERENCE_TEXTURE_LOSS_WEIGHT}"
  --reference-texture-loss-interval "${REFERENCE_TEXTURE_LOSS_INTERVAL}"
  --reference-texture-loss-min-sigma "${REFERENCE_TEXTURE_LOSS_MIN_SIGMA}"
  --reference-texture-loss-max-sigma "${REFERENCE_TEXTURE_LOSS_MAX_SIGMA}"
  --reference-texture-tissue-weight "${REFERENCE_TEXTURE_TISSUE_WEIGHT}"
  --reference-texture-nuclei-weight "${REFERENCE_TEXTURE_NUCLEI_WEIGHT}"
  --reference-texture-composite-weight "${REFERENCE_TEXTURE_COMPOSITE_WEIGHT}"
  --reference-texture-min-pixels "${REFERENCE_TEXTURE_MIN_PIXELS}"
...
if [[ -n "${REFERENCE_VGG_WEIGHTS_PATH}" ]]; then
  TRAIN_TEXTURE_LOSS_ARGS+=(--reference-vgg-weights-path "${REFERENCE_VGG_WEIGHTS_PATH}")
fi
if [[ "${REFERENCE_VGG_RGB}" == "1" ]]; then
  TRAIN_TEXTURE_LOSS_ARGS+=(--reference-vgg-rgb)
fi
