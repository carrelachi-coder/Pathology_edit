...
OUTPUT_DIR="${OUTPUT_DIR:-/data1/zhao/wqx/segmentator_fine/fine_probe_single_scale_seed42}"
COARSE_CHECKPOINT="${COARSE_CHECKPOINT:-/data1/zhao/wqx/segmentator_improved/baseline_seed42/best_composite.pt}"
...
COARSE_MIOU_FLOOR="${COARSE_MIOU_FLOOR:-0.611014}"
COARSE_BOUNDARY_F1_4_FLOOR="${COARSE_BOUNDARY_F1_4_FLOOR:-0.516891}"
...
elif [[ -f "$COARSE_CHECKPOINT" ]]; then
  START_ARGS+=(--init-from-checkpoint "$COARSE_CHECKPOINT")
elif [[ "${ALLOW_RANDOM_INIT:-0}" != "1" ]]; then
  echo "Coarse checkpoint not found: $COARSE_CHECKPOINT" >&2
  echo "Wait for the coarse baseline or set ALLOW_RANDOM_INIT=1 for a smoke test." >&2
  exit 2
fi
...
TRAIN_ARGS=(
  ...
  --seed "$SEED"
  --balanced-datasets
  --dataset-sampling-temperature 0.0
...
echo "Launching coarse-preserving Fine Probe on GPUs $GPU_IDS (world_size=$NUM_GPUS, effective_batch=$((BATCH_SIZE * GRAD_ACCUM_STEPS * NUM_GPUS)))"
if (( NUM_GPUS > 1 )); then
