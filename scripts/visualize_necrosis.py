#!/usr/bin/env python3
"""Visualize necrosis_appearance on real masks.

The script is intentionally portable: if `edit_datasets/` is not present on
the current machine it still writes an empty summary instead of failing.
"""
...
DEFAULT_OUTPUT = Path("phase3_mask_edit/previews/necrosis/real_smoke")
...
def main() -> None:
    ...
    print(f"Wrote {len(rows)} necrosis preview rows to {output_dir}")


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize Phase3 necrosis_appearance on real masks."
    )
