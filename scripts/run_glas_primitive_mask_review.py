...
SCHEMA_VERSION = "glas-cross-meta-primitive-mask-review-v1"
NATIVE_PARTITION_ALGORITHM = (
    "cellvit-clipped-semantic-exact-raster-partition-v2"
)
GRADE_BY_FINE_ID = {
    5: "normal",
    11: "adenomatous",
...
def _eligible_and_score(
    ...
    if primitive_id in NATIVE_REQUIRED_PRIMITIVES:
        return neoplastic >= 8 and row["annulus_free_pixels"] >= 4096, (
            row["annulus_free_pixels"] + 300 * neoplastic
        )
...
def _validate_native_authority(cells_json: Path, semantic_path: Path) -> dict[str, Any]:
    ...
    metrics = {
        ...
        "native_pixel_class_agreement": round(agreement, 8),
    }
    ...
    for key, value in (
        ...
        ("native_pixel_class_agreement", agreement),
    ):
...
def _build_case(
    ...
    *,
    ...
    provenance = {
        ...
        "original_instance_mask_digest": "pending_profile_materialization",
    ...
    if native is not None:
        ...
        provenance["instance_authority_source"] = (
            "frozen_cellvit_native_geometry_semantic_class_binding_v1"
        )
    else:
        provenance["instance_authority_source"] = (
            "cellvit_semantic_mask_shared_watershed_v1"
        )
    ...
    gland_instance = next(
        item for item in produced if item.structure_id == "native_gland_instance_map"
    )
    case = replace(
        case,
        provenance={
            **case.provenance,
            "original_instance_mask_digest": gland_instance.sha256,
        },
    )
