...
CROSS_V2_2_OUTPUT_DIR="${CROSS_V2_2_OUTPUT_DIR:-/data/wqx/flowedit/controlnet_cross_v2_2_hed_texture_degraded}"

# Optional warm start from Cross V1 or Cross V2.2. V1 mask weights are remapped
...
REFERENCE_BANK_BLOCK_SIZE="${REFERENCE_BANK_BLOCK_SIZE:-4}"
REFERENCE_BANK_LABEL_MODE="${REFERENCE_BANK_LABEL_MODE:-tissue_nuclei}"
KEEP_REFERENCE_MASK_FEATURES="${KEEP_REFERENCE_MASK_FEATURES:-0}"
...
TRAIN_CONTROLNET_ARGS=(
  ...
  --reference-bank-block-size "${REFERENCE_BANK_BLOCK_SIZE}"
  --reference-bank-label-mode "${REFERENCE_BANK_LABEL_MODE}"
)
if [[ "${CONTROLNET_TRAIN_X_EMBEDDER}" == "1" ]]; then
  TRAIN_CONTROLNET_ARGS+=(--controlnet-train-x-embedder)
fi
if [[ "${KEEP_REFERENCE_MASK_FEATURES}" == "1" ]]; then
  TRAIN_CONTROLNET_ARGS+=(--keep-reference-mask-features)
fi
...
echo "Cross V2.2 warm-start ControlNet checkpoint: ${CONTROLNET_CHECKPOINT:-<none>}"
echo "Cross V2.2 conditioning checkpoint: ${CONDITIONING_CHECKPOINT:-<none>} load=${LOAD_CONDITIONING_FROM_CHECKPOINT}"
echo "Cross V2.2 output dir: ${CROSS_V2_2_OUTPUT_DIR}"
echo "Cross V2.2 reference block bank: block_size=${REFERENCE_BANK_BLOCK_SIZE} label_mode=${REFERENCE_BANK_LABEL_MODE} keep_ref_mask_features=${KEEP_REFERENCE_MASK_FEATURES}"
echo "Cross V2.2 noising degradation: ${NOISING_DEGRADATION} min_sigma=${DEGRADED_NOISING_MIN_SIGMA} texture_blur=${TEXTURE_BLUR_PROB}/${TEXTURE_BLUR_SIGMA_MIN}-${TEXTURE_BLUR_SIGMA_MAX} downsample=${TEXTURE_DOWNSAMPLE_PROB}/${TEXTURE_DOWNSAMPLE_SCALE_MIN}-${TEXTURE_DOWNSAMPLE_SCALE_MAX} noise=${TEXTURE_NOISE_PROB}/${TEXTURE_NOISE_STD_MIN}-${TEXTURE_NOISE_STD_MAX}"
echo "Cross V2.2 reference perceptual: backend=${REFERENCE_PERCEPTUAL_BACKEND} vgg_weights=${REFERENCE_VGG_WEIGHTS} vgg_weights_path=${REFERENCE_VGG_WEIGHTS_PATH:-<torchvision>} vgg_layers=${REFERENCE_VGG_LAYERS} vgg_loss_type=${REFERENCE_VGG_LOSS_TYPE} same_wsi=${SAME_WSI_APPEARANCE_CHECKPOINT:-<none>} uni=${UNI_CHECKPOINT_PATH:-<none>} weight=${REFERENCE_PERCEPTUAL_LOSS_WEIGHT} sigma=[${REFERENCE_PERCEPTUAL_LOSS_MIN_SIGMA},${REFERENCE_PERCEPTUAL_LOSS_MAX_SIGMA}]"
echo "Cross V2.2 reference grad-ratio interval: ${REFERENCE_GRAD_RATIO_INTERVAL}"

accelerate launch --multi_gpu --num_processes="${NUM_PROCESSES}" --gpu_ids="${GPU_IDS}" \
  controlnet_train/cli/train_controlnet_flux_cross_v2_2.py \
  --pretrained_model_name_or_path "${MODEL_DIR}" \
  --train-metadata "${CROSS_META}" \
  "${TRAIN_HED_ARGS[@]}" \
  "${TRAIN_CHECKPOINT_ARGS[@]}" \
  --output-dir "${CROSS_V2_2_OUTPUT_DIR}" \
  "${RESUME_ARGS[@]}" \
  --logging-dir logs \
  --seed 42 \
  --train-batch-size 1 \
  ...
  --tracker-project-name flux_controlnet_phase5_cross_v2_2_hed_texture_degraded \
