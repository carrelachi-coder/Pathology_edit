...
OUTPUT_DIR="${OUTPUT_DIR:-/data1/zhao/wqx/segmentator_fine/fine_boundary_teacher_joint_seed42}"
TEACHER_CHECKPOINT="${TEACHER_CHECKPOINT:-/data1/zhao/wqx/segmentator_fine/fine_cellvit_teacher_v2_seed42/best_composite.pt}"
BOUNDARY_CHECKPOINT="${BOUNDARY_CHECKPOINT:-/data1/zhao/wqx/segmentator_fine/fine_boundary_v3_seed42/best_composite.pt}"
...
elif [[ -f "$TEACHER_CHECKPOINT" && -f "$BOUNDARY_CHECKPOINT" ]]; then
  START_ARGS+=(
    --init-from-checkpoint "$TEACHER_CHECKPOINT"
    --init-refinement-from-checkpoint "$BOUNDARY_CHECKPOINT"
  )
else
  echo "Joint initialization checkpoints are incomplete:" >&2
  echo "  Teacher: $TEACHER_CHECKPOINT" >&2
  echo "  Boundary: $BOUNDARY_CHECKPOINT" >&2
  exit 2
fi
...
TRAIN_ARGS=(
  ...
  --joint-sampling-fine-fraction 0.6
...
echo "Launching Fine Boundary + CellViT Teacher joint training on GPUs $GPU_IDS (world_size=$NUM_GPUS)"
if (( NUM_GPUS > 1 )); then
