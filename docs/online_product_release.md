...
| Agent loop | canonical runner | The UI writes a per-run or per-patch `cell_fill_log.json`; the runner validates every nuclei attempt, reason code, bounded gamma update, frozen evaluation gamma, count, type routing, shape, erasure, buffer, spacing and checkpoint provenance before image generation. The nuclei loop allows at most three attempts and at most one parameter update. It also enforces `28` inference steps, guidance `3.5`, ControlNet scale `1.0`, `bf16`, seed `42`, routing thresholds `0.12/0.30`, the frozen legacy-anchor release and P1 `shadow`. Semantic routing uses the exact source/target tissue difference; the subsequent image-generation loop still allows at most two attempts. |
