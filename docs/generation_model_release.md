...
spatial landing score. Counts, density calibration and exact nucleus-type
...
| `Qinxin11/pathology-segmentator` | C-line Boundary + CellViT Teacher joint epoch-2 state dict, strict `release.json`, manifest and model card | `/data1/zhao/wqx/segmentator_fine/fine_boundary_teacher_joint_seed42/best_composite.pt` |
...
directory with `hf upload-large-folder`. Package the Segmentator state dict
with `benchmark_configs/releases/segmentator_fine_c_epoch2.json` as
`release.json` in the separate private Segmentator repository. When `amax2`
cannot reach Hugging Face directly, copy the frozen artifact to an authenticated
client, verify SHA256 before upload, and verify Hub LFS metadata afterward.
...
- Hub API verification on 2026-07-29 reports all four repositories as private.
  The newly published ProbNet and Segmentator checkpoints were hashed after
  transfer from `amax2`; Hub LFS metadata reports the same exact byte sizes and
  SHA256 values.
...
| C-line Segmentator joint epoch 2 | 2,820,003,238 | `ddb95a7be8a2d80f6d8a77672b265f261fbe79ca2c3e35215c9ed871da604c34` |
