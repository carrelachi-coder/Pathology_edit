...
class Pix2PixPostprocessInferenceTests(unittest.TestCase):
    def test_cross_low_stain_protection_preserves_bright_cell_free_component(self):
        ...
        self.assertEqual(info["policy"], "cross_rgb_od_near_white_v2")
        self.assertAlmostEqual(info["minimum_luma"], 0.90)
        self.assertAlmostEqual(info["minimum_rgb_channel"], 0.86)
        self.assertAlmostEqual(info["maximum_chroma"], 0.10)
        self.assertAlmostEqual(info["maximum_otsu_od"], 0.12)
    ...
    def test_epoch25_settings_restore_identity_but_ignore_retired_trust_gate(self):
        ...
        self.assertTrue(config.class_reference_trust_enabled)
        self.assertAlmostEqual(config.class_reference_fallback_scale, 0.05)
        self.assertAlmostEqual(config.class_reference_matched_tissue_floor, 0.05)
        self.assertAlmostEqual(config.same_wsi_identity_low_support_max_gain, 1.50)
