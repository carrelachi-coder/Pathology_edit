...
def independent_focus_minimum_center_separation_px(
    ...
    """Return the minimum center separation certified before execution.

    The final gates reconstruct spatial foci from accepted centers. Preflight
    must therefore use the same separation ruler for every primitive whose
    final contract requires independent foci. Peritumoral scatter and budding-
    like clusters retain their primitive-specific rulers; cellularity increase
    uses the generic three-diameter spatial-focus ruler.
    """

    if primitive_id == "peritumoral-neoplastic-scatter-increase-v1":
        separation_diameters = SCATTER_MINIMUM_CENTER_SEPARATION_DIAMETERS
    elif primitive_id == "peritumoral-small-cluster-increase-v1":
        separation_diameters = SMALL_CLUSTER_BETWEEN_FOCUS_SEPARATION_DIAMETERS
    elif primitive_id == "cellularity-increase-v1":
        separation_diameters = (
            CELL_EFFECT_MINIMUM_INTER_FOCUS_SEPARATION_DIAMETERS
        )
    else:
        return 0.0
    return float(separation_diameters) * max(
